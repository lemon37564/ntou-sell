...
func (ser *Server) setCookies(w http.ResponseWriter, r *http.Request, account, password string) {
	...
	cookie := &http.Cookie{Name: "se-ss-account", Value: account, Expires: expire, Secure: false}
	cookie2 := &http.Cookie{Name: "se-ss-password", Value: password, Expires: expire, Secure: false}
...
func (ser *Server) getCookies(w http.ResponseWriter, r *http.Request) (*http.Cookie, *http.Cookie, bool) {
	...
	log.Println("cookie(pass):", cookie2, "err:", err2)
